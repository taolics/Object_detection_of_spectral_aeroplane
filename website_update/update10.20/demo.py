...
def parse_args():
    ...
    parser.add_argument('inputimg',type = str)
...
if __name__ == '__main__':
    args = parse_args()
    file = "/var/www/html/upload/"+str(args.inputimg)
    # model path
    demonet = args.demo_net
    dataset = args.dataset
    tfmodel = os.path.join('output', demonet, DATASETS[dataset][0], 'default', NETS[demonet][0])
    tfmodel = '/var/www/html/python/Faster-RCNN-TensorFlow-Python3/' + tfmodel
    ...
    demo(sess, net, file)
    path = '/var/www/html/download/result.png'
    if os.path.exists(path):  # 如果文件存在
    # 删除文件，可使用以下两种方法。
        os.remove(path)
    plt.savefig('/var/www/html/download/result.png')
    plt.show()
